...
4. **Finish with:**
   ```
   Yo [name], system ready. Start whenever you are.
   ```
...
3. **Output** — loading bar + compact validation:
   ```
   ▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓  100%
   ✓ CORE · ENV · memory · TRANSFER | [name]
   Yo [name], system ready.
   ```
4. **Display** TRANSFER content visibly (below the greeting — shows current session context inline)
...
> Other greetings are treated as a casual conversation starter without ceremony.
